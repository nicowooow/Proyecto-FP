...
class linkRepository {
	 ...
	 */
	createLink(profile_id, title, url, url_image, position, is_visible) {
		const sql =
			"insert into links (profile_id, title, url,url_image, position, is_visible) values ($1 ,$2, $3, $4, $5,$6)";
		return pool
			.query(sql, [profile_id, title, url, url_image, position, is_visible])
	 ...
	 * @returns {Promise<number>} filas afectadas
	 */
	putLink(id, title, url, url_image, position, is_visible) {
		const sql =
			"update links set title = $1, url = $2, url_image = $3, position = $4, is_visible = $5 where id = $6 ";
		return pool
			.query(sql, [title, url, url_image, position, is_visible, id])
			.then(({ rowCount }) => rowCount);
	}
	 ...
	 * @returns {Promise<number>} filas afectadas
	 */
	patchLink(id, title, url, url_image, position, is_visible) {
		const sql =
			"update links set title = COALESCE($1,title) , url = COALESCE($2,url) , url_image = COALESCE($3,url_image) , position = COALESCE($4,position) , is_visible = COALESCE($5,is_visible) where id = $6 ";
		return pool
			.query(sql, [title, url, url_image, position, is_visible, id])
			.then(({ rowCount }) => rowCount);
	}
