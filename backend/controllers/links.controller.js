...
export const post_link = async (req, res) => {
  try {
    let {profile_id, title, url, position, is_visible} = req.body;
    let rowCount = await linkRepository.createLink(profile_id, title, url, position, is_visible);
    if (rowCount > 0) {
...
export const put_link = async (req, res) => {
  try {
    let { id } = req.params;
    let {title,url,position,is_visible} = req.body;
    console.log(id,title,url,position,is_visible);
    
    let rowCount = await linkRepository.putLink(id, title, url, position, is_visible);
    if (rowCount > 0) {
...
export const patch_link = async (req, res) => {
  try {
    let { id } = req.params;
    let {title,url,position,is_visible} = req.body;
    let rowCount = await linkRepository.patchLink(id, title, url, position, is_visible);
    if (rowCount > 0) {
